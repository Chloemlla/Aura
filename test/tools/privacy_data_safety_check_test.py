...
class PrivacyDataSafetyCheckTest(unittest.TestCase):
    ...
    def test_accepts_allowlist_rules_that_omit_the_declared_store(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = seed_repo(Path(tmpdir))
            policy = minimal_policy()
            write(
                repo / "app/src/main/res/xml/backup_rules.xml",
                '<full-backup-content><include domain="sharedpref" path="included.xml" /></full-backup-content>\n',
            )
            write(
                repo / "app/src/main/res/xml/data_extraction_rules.xml",
                '<data-extraction-rules><cloud-backup>'
                '<include domain="sharedpref" path="included.xml" />'
                '</cloud-backup><device-transfer>'
                '<include domain="sharedpref" path="included.xml" />'
                '</device-transfer></data-extraction-rules>\n',
            )
    ...
    def test_rejects_allowlist_that_backs_up_a_declared_exclusion(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = seed_repo(Path(tmpdir))
            policy = minimal_policy()
            write(
                repo / "app/src/main/res/xml/backup_rules.xml",
                '<full-backup-content><include domain="sharedpref" path="test-store.xml" /></full-backup-content>\n',
            )
            write(
                repo / "app/src/main/res/xml/data_extraction_rules.xml",
                '<data-extraction-rules><cloud-backup>'
                '<include domain="sharedpref" path="test-store.xml" />'
                '</cloud-backup><device-transfer>'
                '<include domain="sharedpref" path="test-store.xml" />'
                '</device-transfer></data-extraction-rules>\n',
            )

            with self.assertRaisesRegex(PrivacyDataSafetyError, "is missing from"):
                validate_policy(repo, policy)
