...
class ThemePackRecipeManagerTest {
    ...
    fun `wallpaper pack remap replaces embedded local daypart slots`() {
        ...
        val remapped = remapWallpaperPackAssetLocators(
            ...
            assetRemaps = mapOf("assets/morning.jpg" to "/data/user/0/com.freevibe/files/theme_packs/morning.jpg"),
        )

        val parsed = parsePack(remapped)

        assertEquals("/data/user/0/com.freevibe/files/theme_packs/morning.jpg", parsed?.slots?.first()?.wallpaperUri)
