 ...
 * - `com.chloemlla.aura.action.ROTATE_NOW` - enqueue a one-shot wallpaper rotation
 *   that respects the user's existing rotation source + target prefs.
 * - `com.chloemlla.aura.action.SHUFFLE_NOW` - alias for ROTATE_NOW.
 * - Optional extra `com.chloemlla.aura.extra.CALLER_PACKAGE` records the automation
 ...
 *
 * ```
 * Send Intent -> Action: com.chloemlla.aura.action.ROTATE_NOW
...
class TaskerActionReceiver : BroadcastReceiver() {
    ...
    companion object {
        const val ACTION_ROTATE_NOW = "com.chloemlla.aura.action.ROTATE_NOW"
        const val ACTION_SHUFFLE_NOW = "com.chloemlla.aura.action.SHUFFLE_NOW"
    }
}
