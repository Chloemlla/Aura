...
EXPECTED_DISTRIBUTION_URL = r"https\://services.gradle.org/distributions/gradle-9.5.0-bin.zip"
...
def validate_gradle_wrapper(path: Path) -> dict[str, Any]:
    ...
    if distribution_url != EXPECTED_DISTRIBUTION_URL:
        raise GradleWrapperPolicyError("Gradle wrapper distributionUrl must remain the reviewed Gradle 9.5.0 bin ZIP")
    ...
    if distribution_sha256 != EXPECTED_DISTRIBUTION_SHA256:
        raise GradleWrapperPolicyError("Gradle wrapper distributionSha256Sum does not match the reviewed Gradle 9.5.0 checksum")
