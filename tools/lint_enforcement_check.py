...
A lint baseline is permitted, but only as a committed, reviewable inventory: the
fork's Android 17 upgrade (AGP 9.3.1 / Compose BOM 2026.06 / media3 1.11) newly
flagged a few hundred findings on pre-existing code, and absorbing them in a
tracked file still fails the build on any NEW error. What is not permitted is a
`baseline =` pointing at a file that is not there: AGP writes one on the first
run and silently swallows whatever it finds, which is the muzzled-lint failure
...
COLORS_HELPER_SOURCE = "app/src/main/java/com/chloemlla/aura/service/LiveWallpaperColors.kt"
ENGINE_SOURCE_ROOT = "app/src/main/java/com/chloemlla/aura/service"
...
BASELINE_ENTRY = re.compile(r'^\s*baseline\s*=\s*file\("([^"]+)"\)', re.MULTILINE)
BASELINE_ISSUE = re.compile(r"<issue\b")
...
def validate_lint_enforcement(repo_root: Path) -> dict[str, object]:
    ...
    baseline_match = BASELINE_ENTRY.search(block)
    baseline_path = baseline_match.group(1) if baseline_match else None
    baseline_issues = 0
    if baseline_path is not None:
        # The declaration is resolved relative to the module, not the repo root.
        resolved = repo_root / Path(BUILD_SCRIPT).parent / baseline_path
        if not resolved.is_file():
            errors.append(
                f"the lint block declares baseline {baseline_path}, which is not "
                "committed; AGP would write it on the first run and swallow every "
                "finding it captured without review"
            )
        else:
            baseline_issues = len(BASELINE_ISSUE.findall(resolved.read_text(encoding="utf-8")))
            if baseline_issues == 0:
                errors.append(
                    f"the lint baseline {baseline_path} records no issues, so it "
                    "only adds a way to hide findings later"
                )
    ...
    return {
        ...
        "detectorDisables": 0,
        "lintBaselineDeclared": baseline_path is not None,
        "lintBaselinePath": baseline_path,
        "lintBaselineIssues": baseline_issues,
