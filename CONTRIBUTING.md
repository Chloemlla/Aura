...
signing.keystore.path=../aura.jks  # release builds only
...
- Unit tests live in `app/src/test/java/com/chloemlla/aura/` and run via `testFullDebugUnitTest`. Pure-Kotlin code (geometry helpers, content filters, query builders) should land with tests; ViewModels can rely on integration coverage if mocking is heavy.
